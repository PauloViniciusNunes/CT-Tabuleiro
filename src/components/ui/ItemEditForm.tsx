...
const ItemEditForm: React.FC<ItemCreateProps> = ({
  availableCards,
  theseItem,
  onSave,
  onClose,
}) => { 
  const [slot, setSlot]     = useState<ItemSlot>(theseItem.slot);
  const [rarity, setRarity] = useState<ItemRarity>(theseItem.rarity);

  const [ocasionalAdd, setOcasionalAdd] = useState(theseItem.ocasionalAdd);
  const [atribute, setAtribute] =
    useState<keyof Omit<TokenAttributes, "level" | "xp">>(theseItem.atributeToOcasionalAdd);

  const [selectedCards, setSelectedCards]   = useState<Card[]>(theseItem.habilityCards ?? []);
  const [cardPickerOpen, setCardPickerOpen] = useState<boolean>(false);
  const [craftable, setCraftable] = useState(theseItem.craftable);

  const [itemName, setItemName]   = useState<string>(theseItem.name)
  const [itemDesc, setItemDesc]   = useState<string>(theseItem.desc)

  const [itemImgUrl, setItemImgUrl]   = useState<string>(theseItem.imgUrl)
  const [itemValue, setItemValue]     = useState<number>(theseItem.value)
