...
export const TokenEditForm: React.FC<TokenFormProps> = ({theseToken, onSave, onClose, cards, items }) => {
  const [name, setName] = useState(theseToken.name);
  const [imagePreview, setImagePreview] = useState<string | null>(theseToken.imageUrl);
  const [attributes, setAttributes] = useState<TokenAttributes>(theseToken.attributes);
  const [proficiencies, setProficiencies] = useState<TokenProficiencies>(
    theseToken.proficiencies
  );
  const [inventory, setInventory] = useState<TokenInventory>(theseToken.inventory);
  const [status, setStatus]       = useState<TokenStatus>(theseToken.status);
  const [team, setTeam]           = useState<TokenTeam>(theseToken.team);

  const [bodytobodyRange, setBodytobodyRange] = useState(theseToken.bodytobodyRange);
  const [magicalRange, setMagicalRange]       = useState(theseToken.magicalRange);

  const [tokenClass, setTokenClass]               = useState<TokenClass>(theseToken.class);
  const [primaryElement, setPrimaryElement]       = useState<TokenPrimaryElement>(theseToken.tokenPrimaryElement ?? "neutro");
  const [primaryDisvantage, setPrimaryDisvantage] = useState<TokenPrimaryDisvantage>(theseToken.tokenPrimaryDisvantege ?? "none");
  
  const [selfCards, setSelfCards] = useState<Card[]>(theseToken.cards);
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) {
      setImagePreview(theseToken.imageUrl);
      return;
    }
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const token: Token = {
      ...
      team,
      class: tokenClass,
      cards: selfCards, // Aqui
      position: { col: theseToken.position.col, row: theseToken.position.row },
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-3">
      ...
      >
        <h2 className="text-2xl font-bold text-green-400">Editar Token</h2>
        ...
        <fieldset className="border border-gray-600 p-3 rounded bg-gray-700 bg-opacity-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
            ...
            <fieldset className="border border-gray-600 rounded p-2">
              ...
              <button
                type="button"
                onClick={() => setItemChooseOpen(true)}
