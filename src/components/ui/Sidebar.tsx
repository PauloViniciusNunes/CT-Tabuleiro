    ...
    <>
      <div className="h-full bg-gray-900 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {activeTab === "chat" && (
            <div className="flex flex-col h-full">
              <h2 className="text-lg font-bold text-white select-none mb-2">Historic Battle</h2>
              <div className="flex-1 overflow-y-auto bg-gray-800 rounded p-2 mb-2 space-y-1">
                {battleHistory.length === 0 ? (
                  ...
                  battleHistory.map((act, idx) => (
                    <div key={idx} className="text-xs text-white">
                      {`[Round ${act.rollResult ? act.rollResult.rawRolls.length : ""}] `}
                      <span className="font-semibold">{act.attribute}</span>
                      {` - ${act.type} = ${act.rollResult.total}`}
                    </div>
                  ))
                )}
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  disabled
                />
                <button className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded text-white text-sm font-bold" disabled>
                  Filtrar
                </button>
              </div>
            </div>
