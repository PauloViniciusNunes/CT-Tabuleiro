import React, { useEffect, useEffectEvent, useRef, useState, type ChangeEvent, type ChangeEventHandler, type FormEvent } from "react";
...
export const CardEditForm: React.FC<CardEditProps> = ({
  theseCard,
  onSave,
  onClose,
}) => {
  const [imagePreview, setImagePreview] = useState<string | null>(theseCard.img);
  const [description, setDescription] = useState(theseCard.desc);
  const [causality, setCausality] = useState(theseCard.causality);
  const [causalityType, setCausalityType] = useState<CardCausality>(theseCard.causalityType);
  const [actionsRequired, setActionsRequired] = useState(theseCard.actionsRequired ?? 1);
  const [typeTarget, setTypeTarget] = useState<string>(theseCard.target.type);
  const [numbersTarget, setNumbersTarget] = useState<number>(theseCard.target.numbersTarget ?? 1);
    useEffect(() => {
      setNumbersTarget(1);
  },[typeTarget]);
  ...
  const [numbersEntity, setNumbersEntity] = useState<number>(theseCard.entityQuantity);
  const [cardDuration, setCardDuration] = useState<CardDuration>(theseCard.duration ?? 4);
  const [isSpell, setIsSpell]           = useState<boolean>(theseCard.spellCircle === undefined ? false: true);
  const [spellType, setSpellType]       = useState<SpellType>(theseCard.spellType ?? "Abjuração");
  const [spellCircle, setSpellCircle]   = useState<SpellCircle>(theseCard.spellCircle ?? 1);
  const [haveEffectApplication, setHaveEffectApplication] = useState<boolean>(theseCard.effectToApply.length > 0 ? true : false);
  const [effectApplication, setEffectApplication] = useState<EffectType[]>(theseCard.effectToApply);

  const [haveDuration, setHaveDuration] = useState<boolean>(theseCard.duration !== undefined ? true : false);
  const [cardRecharge, setCardRecharge] = useState(Number(theseCard.recharge ?? 4));
  
  /* Configuráveis de Pivot */
  const [pivotImgUrl, setPivotImgUrl] = useState<string>(theseCard.target.pivotSettings?.areaImgUrl ?? "");
  const [pivotType, setPivotType]     = useState<PivotType>(theseCard.target.pivotSettings?.pivotType ?? "Trigger-Fix");
  const [pivotCellRange, setPivotCellRange] = useState<number>(theseCard.target.pivotSettings?.range ?? 1);
  /* * */
  ...
  const [cardName, setCardName] = useState<string>(theseCard.name);
  const [useBaseDice, setUseBaseDice]   = useState(theseCard.baseDice !== null ? true : false);
  const [diceQuantity, setDiceQuantity] = useState(theseCard.baseDice?.quantity ?? 1);
  const [diceType, setDiceType] = useState<DiceType>((theseCard.baseDice?.type ?? "d6") as DiceType);
  const [isPartilOffensive, setIsPartialOffensive] = useState<boolean | undefined>(theseCard.partialOffensive);

  const [useManaScale, setUseManaScale] = useState(false);
  const [manaScale, setManaScale]       = useState<ManaCostScale>((theseCard.manaRequired ?? "Normal") as ManaCostScale);
  useEffect(() =>
  {
