    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-gray-800 p-4 rounded-lg w-full max-w-md text-white space-y-4">
        ...
        <div>
          ...
          <select
            value={type}
            onChange={(e) => setType(e.target.value as MapObjectType)}
        ...
        {/* Item relativo */}
        <div>
          <span className="text-sm font-semibold">Item Relativo</span>

          <button
            onClick={() => setItemPickerOpen((prev) => !prev)}
            className="w-full mt-1 p-2 bg-gray-700 hover:bg-gray-600 rounded border border-gray-600 text-left"
          >
            {selectedItem ? selectedItem.name : "Selecionar item..."}
          </button>
            ...
            <div className="max-h-[200px] overflow-y-auto mt-2 space-y-2">
              {createdItems.length === 0 ? (
                ...
                createdItems.map((item) => {
                  const alreadySelected = selectedItem?.id === item.id;
