...
export class RemoveMemberCampaignService {
    ...
    async execute(data: unknown) {
        ...
        if (!campaign)
            throw new Error("Não foi possível encontrar a campanha para RemoveMemberCampaignService.")

        const user = await this.userRepository.findById(newData.userId)

        if (!user)
            throw new Error("Não foi possível encontrar usuário para RemoveMemberCampaignService.")

        const campaignId: string = newData.campaignId
        const userId: string     = newData.userId

        await this.repository.removeMember(campaignId, userId)
    }
}