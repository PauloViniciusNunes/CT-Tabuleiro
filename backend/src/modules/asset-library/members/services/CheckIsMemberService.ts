...
export class CheckIsMemberService {
    ...
    async execute(data: unknown) {
        ...
        if (!campaign)
            throw new Error("Não foi possível encontrar a campanha para CheckIsMemberService.")

        const user = await this.userRepository.findById(newData.userId)

        if (!user)
            throw new Error("Não foi possível encontrar usuário para CheckIsMemberService.")

        const campaignId: string = newData.campaignId
        const userId: string     = newData.userId

        return await this.repository.isMember(campaignId, userId)
    }
}